...
「发现」中的 X Lists 是所有自媒体工作空间固定具备的列表管理与情报接入工作台，不是通用 X 客户端。它从当前 WMB 安装共享的专用 Edge 登录态实时读取三类可访问 List：用户创建、用户关注、用户在其中。稳定身份是 workspace ID、当前安装账号、X List ID 与规范 URL；名称不是身份。WMB 只在当前根持久化账号快照、用户绑定为发现信源的 List 及每次操作的证据，不镜像全部成员或完整动态历史。
