...
- Crew revision: 2026-08-08 crew multi-instance invariants (§1.0 items 8-9), CAP-027 Desk manager + multi-instance crew runtime, EVAL-030; design `docs/spark/2026-08-08-agent-crew-multi-instance-design.md` (正式施工 Owner lock; TASKS doing 仍是唯一施工许可)
- Research revision: 2026-08-10 ResearchJob evidence supplement + single-hop successor narrow exception (§1.0 item 9, §2.2), CAP-028, EVAL-032; milestone M-5170; design `docs/spark/2026-08-10-agent-research-job-design.md` (正式 Owner lock 2026-08-10; 仅授权 Legislate, 不授权实现)
- Zhihu hot loop revision: 2026-08-22 Zhihu hot question driven daily article×video-script closed loop (PRD §2.6), CAP-029 + EVAL-033, milestone M-5330; design `docs/spark/2026-08-22-zhihu-hot-question-content-loop-PLAN.md` (正式 Owner lock 2026-08-22; WMB-5330 只立法与共享基础，不实现浏览器/Today/Studio/调度)
- Product source: `PRD.md` (§2.0 form, §2.3 roles, §2.4 crew multi-instance) and `PRODUCT.md`
...
8. **Fixed roles are labor-division identities, not seats (role ≠ slot)**: the five roles (desk/reporter/planner/writer/librarian) are lane-stable job types that carry no slot-count semantics; the same role may run multiple concurrent instances, each created per task and leaving the active view at terminal state (`needs_user` stays until the human closes it). Active views must never draw empty seats: the five role groups are always visible and an empty role renders 「当前无任务」.
9. **Shared pool is capacity, not quota; desk is the software-side supervisor workstation (主管/主编席) with full internal standing write power; red lines are exactly three categories**: `maxWorkers` (0..7, 0 disables dispatch, default 2) is the all-role shared concurrency cap and must not be read as per-role staffing or quota; **desk (the software supervisor) holds full internal standing write power = union of all `agentGrantable` business-capability commands ∪ `INFRA_GRANT_COMMANDS` (including internal prepare commands, excluding redline-category execution commands)**; any page dock (publish and agents pages included) bound to the supervisor receives a grant covering the full internal command set; internal approvals belong to the supervisor (2026-08-10 supervisor-authority flip, Owner lock); the human Owner is not the software supervisor — the human's role is fresh confirmation and final liability for exactly three redline **categories** (final platform publish, hard-delete execution, external platform change execution), and the supervisor may prepare but never execute redline final acts (human-UI fresh confirmation only, `agentGrantable: false`); desk never occupies an employee slot (`spawn(roleId:'desk')` → `ROLE_NOT_SPAWNABLE`). Instance first-class identity is `jobId`; display numbering is active-period-only; the spawn contract persists jobId/roleId/brief/boundary params in the existing `context_refs_json` for restart re-dispatch. See CAP-027. **The ResearchJob automatic single-hop successor (CAP-028) is the sole system-wide narrow exception to single-hop dispatch**: after a research task reaches terminal state and produces an EvidencePack, the research terminal-state handler creates exactly one new `research_successor` job for the original parent role — a new work order (same boundary, same roleId, new jobId), never a retry of the research job; research→research chaining is forbidden, and a further material gap after the successor turns to `needs_user` for human decision.
10. **Librarian topic maintenance is proposal-first; approval is supervisor-internal**: Agents may prepare exact topic change sets, but formal topic/domain structure and linked-object relations change only after supervisor (desk) internal approval (2026-08-10 supervisor-authority flip; `knowledge.topic_maintenance_approve / reject / reproposal_retry` are `agentGrantable:true`, `precise:false`, `{desk:true}`). The supervisor presents the proposal and performs the internal approval; it never substitutes manual Owner editing for employee work, and the human Owner's role is the three redline categories only. See CAP-003 and `docs/spark/2026-08-10-topic-maintenance-approval-design.md`.
...
8. Roles are identity types, not seats: a role is not bound to a slot count and the same role may own multiple concurrent instances. Per-instance semantics (first-class `jobId`, lifecycle, permission intersection, dispatcher object gate, persistence contract, `needs_user` and empty-state behavior) are specified in CAP-027, normative companion `docs/spark/2026-08-08-agent-crew-multi-instance-design.md` (formal construction Owner lock; TASKS doing remains the only construction permit).
9. `librarian` owns topic-housekeeping proposals (create/update/merge/archive/reassign) but never applies them. Topic proposal application/rejection is **supervisor-internal** (desk: `agentGrantable:true`, `precise:false`, `defaultRoleBindings { desk: true }`) and must not become employee standing role power (2026-08-10 supervisor-authority flip, Owner lock); planner keeps topic/opportunity judgment without bypassing the same approval boundary for formal topic-structure changes.
...
- The active runtime serializes business writes; no last-write-wins merge exists. Bounded read/research work may run concurrently only while it does not bypass this write boundary. A research task's 3 parallel fetches (CAP-028) are bounded reads inside one worker: they are governed by an in-task concurrency signal that holds exactly one worker slot, they do not consume or interact with the `maxWorkers` quota, and every write still serializes through this write boundary.
...
Topic maintenance uses a durable change-set ledger. A librarian proposal freezes an ordered before/after payload and one explicit intent-level conflict contract generated by the same canonical builder used for approval. The contract covers only facts whose change can alter this approval's result: directly changed topic revisions, create/update canonical-key availability, exact merge memberships and revisions, and exact reassign source/target relation keys; unrelated graph drift is ignored. Proposal creation changes no formal topic fact. Supervisor (desk) rejection changes only proposal status. Supervisor (desk) approval applies the entire set in one transaction: create/update/merge/archive/reassign operations either all succeed or write nothing (2026-08-10 supervisor-authority flip: topic approval is supervisor-internal, `agentGrantable:true`/`precise:false`/`{desk:true}`). A real conflict atomically marks the immutable proposal stale, records structured evidence and inserts one durable topic-reproposal outbox item; only after commit may the system dispatch a librarian to re-read current facts and create a distinct pending proposal linked by `supersedes_proposal_id`. The stale proposal is never revived, rebased or auto-approved. Merge migrates all formal source, plan, content-project and review links to the retained topic and archives, rather than hard-deletes, the merged topic. Legacy stale proposals remain historical and are not auto-enqueued. The complete proposal/supersession ledger is readable on Topic; Today projects only pending decisions. Desk must present pending proposals and may not instruct the Owner to perform the underlying edits manually.
...
Only explicit user intents that require research, judgment, writing, rewriting, or review create Pi tasks. Starting or continuing such a task is itself the Owner's authorization for Pi and task-scoped collaborators to perform the bounded business writes required by that intent: WMB automatically binds the worker lease and least-privilege `TaskGrantV1`, and must not add a second "authorize AI collaboration" confirmation or expose grant administration as a normal workflow. Deterministic UI actions continue to call existing business commands directly. Task grants never authorize final publication, account/profile changes, source-configuration changes, X List mutations, or other browser/platform side effects; those retain their exact UI confirmation boundaries.
...
### CAP-027 Desk manager + multi-instance crew runtime

Links: REQ-028, REQ-029, AC-024, AC-025, AC-026, AC-027.
Normative companion: `docs/spark/2026-08-08-agent-crew-multi-instance-design.md` (formal construction Owner lock; TASKS doing remains the only construction permit). Implements PRD §2.4, PRODUCT C9 and SPEC §1.0 items 8-9 on top of the M-5110 JobPool runtime.

1. **Role ≠ slot; instance = jobId.** `desk`, `reporter`, `planner`, `writer`, `librarian` are labor-division identity types with no slot-count semantics. An **instance** is one task-run execution unit with first-class identity `jobId` + immutable `roleId`, an active-period employee lease, a task grant and an isolated Pi session (`job-<jobId>.jsonl`); it is created by enqueueing a spawned job and binds exactly one role. Multiple instances of the same role must be able to exist and run concurrently, each with independent context, session and grant; instances never inherit other instances' state, and one task belongs to at most one active instance at a time (scan→judge may rebind the same `agent_task`; progress/needs_user projection must not double-count).
2. **Instance lifecycle.** Create = enqueue: `queued`, `waiting_resource` and `running` are visible in the active view; terminal `succeeded` / `partial` / `failed` / `cancelled` exit the active view immediately; terminal `needs_user` stays in the active view (§4). Terminal order must be: agent_task terminal → grant revoke via the agent_task terminal hook (idempotent; cancel must not explicitly revoke) → lease/entity-lock release → pool terminal + `JOB_EVENT`; exception: a scan→judge handoff may terminate the job before the shared `agent_task` terminal. Cancel is priority over every outcome (no path may land succeeded/failed/partial/needs_user after `signal.aborted`), is idempotent, and its total gate is ≤5s. `waiting_resource` consumes no slot and remains cancellable, readable and messageable.
3. **Instance projection (no empty seats).** The active view (Agents page, Today duty strip, department pages) must render instances, never seats: the five role groups are always visible; a role with no instance renders exactly 「当前无任务」 and must not draw empty slots, placeholder seats or a fictional 「待命」; the page-header summary shows 工作中 N · 排队 M · 等你批 K totals. Display numbering (「记者 #2」) is active-period-only, never enters contracts or storage and restarts the counter on restart; persistent cross-restart identification uses `jobId` only. History must be rebuilt from the persistent surface only (`agent_tasks`, `context_refs_json`, session files, audit), never from the in-memory JobPool, and must offer one-click re-dispatch (rebuild the original `RoleJobRequest`).
4. **needs_user precise behavior.** A `needs_user` instance is a terminal state that stays in the active view under 「等你批」 until the user handles or closes it; during that time it must hold no concurrency slot, no lease, no grant and no lock, must not auto-retry, and must carry a stable `code` plus any partial readback evidence. The desk reports 「需要你 X」; after a human action the desk re-dispatches or closes the instance. Copy must never describe it as failed or as waiting on capacity.
5. **Shared pool = pure capacity.** `maxWorkers` (0..7, 0 disables dispatch, default 2) is the all-role shared cap on simultaneously running jobs; it is a capacity knob, not per-role staffing or quota, and raising it must never require role/registry changes. `spawn` must reject when `maxWorkers = 0`. Business writes keep CommandDispatcher + entity-lock + `expected_revision` semantics (REVISION_CONFLICT unchanged); lock conflicts park as `waiting_resource` (`RESOURCE_LOCK_CONFLICT` / `RESOURCE_LEASE_BUSY` / `RESOURCE_JUDGE_IN_FLIGHT`) with promotion events plus a 60s watchdog.
6. **Desk = the software-side supervisor workstation (主管/主编席) with full internal standing write power; the human is not the software supervisor.** Desk (the software supervisor) holds full internal standing write power = `commandsCoveredByGrantableCapabilities() ∪ INFRA_GRANT_COMMANDS` (including internal prepare commands, excluding the three redline-category execution commands); any page dock (publish and agents pages included) bound to the supervisor receives a grant covering the full internal command set; internal topic approval belongs to the supervisor (2026-08-10 supervisor-authority flip, Owner lock). The human Owner's role is fresh confirmation and final liability for exactly three redline **categories** (final platform publish, hard-delete execution, external platform change execution); the supervisor may prepare but never execute redline final acts. Desk never occupies an employee slot (`spawn(roleId:'desk')` → `ROLE_NOT_SPAWNABLE`); its tools include orchestration (roster/jobs/message) plus the full internal write set, and its answers come only from projection APIs. Dispatch is single-hop: the desk hands one bounded job to one instance, and instances must not re-dispatch to each other (no orchestration graph, no auto multi-hop). The sole system-defined exception is the ResearchJob single-hop successor authorized by CAP-028: the research terminal-state handler (system, not an instance and not the desk) creates one new `research_successor` job for the original parent role; it is a new work order, not instance-to-instance re-dispatch, and the general single-hop rule here stays intact.
7. **Instance permission = task grant ∩ role capability ∩ resource boundary; object gate is construction-mandatory.** Each instance's effective write set is the intersection of its task grant scope, its role's enabled capabilities and its resource boundary carried by `RoleJobSpec.resourceLocks` (businessDate / projectId / sourceIds / scope). The dispatcher must validate the job's object boundary on both the grant-issue and execution paths and must reject cross-object write requests with BLOCKED plus audit; grant existence or `expected_revision` alone must not be treated as a boundary defense. Red lines are unchanged: final platform publish, hard-delete and platform side effects are `agentGrantable: false` and unreachable by any instance/grant combination.
8. **Persistence contract (restart re-dispatch).** The spawn contract must write jobId/roleId/brief/boundary params into the existing `context_refs_json` column; no new table or column is allowed, `agent_tasks` / `task_grants` / `execution_grants` schemas stay unchanged, and JobPool stays in-memory. After a restart the UI must be able to fully identify a historical instance from `context_refs_json` + `agent_tasks` + session file + audit and offer one-click re-dispatch that rebuilds the original `RoleJobRequest`. CAP-028 authorizes the single new business table `research_claims` (research evidence verdict table, query/audit surface) as the only exception to the no-new-table rule here: it is a business fact table, not a successor-contract table — the research successor contract itself persists only in `context_refs_json` / `progress_json` / `checkpoint_json` / `result_refs_json`, so this item and the general rule are not in conflict.
9. **Compatibility and construction gates.** No generic role, no cloud service, no multi-tenant model, no new platform API, no configurable permission UI before P0 (registry + role filtering + readonly roster), no permanent-employee entities. The Capability registry (`src/shared/agent-capabilities.ts`, `page-authority.ts`) stays zero-change for CAP-027 itself except the object-gate enforcement point; CAP-028 is the only authorized extension and is limited to `cap.research`, its optional read whitelist and `research` intent projections. Role Skills / lane packs still carry no commands or role bindings. Construction must pass `npm run check:capabilities` (G1) and effective-grant consistency (G2) or the build fails.

### CAP-028 ResearchJob evidence supplement and single-hop successor

Links: REQ-002, REQ-031, AC-001, AC-029.
Normative companion: `docs/spark/2026-08-10-agent-research-job-design.md` (formal Owner lock 2026-08-10; legislation authorized, implementation not authorized). Implements PRD §2.5, the PRODUCT C9 narrow-exception clause (C9.2 amendment) and C9.10 scope clarification, plus SPEC §1.0 item 9 and §2.2, on top of the CAP-027 crew runtime; milestone M-5170.

1. **cap.research and the reporter as the only researcher.** The Capability registry (`src/shared/agent-capabilities.ts`) gains `cap.research`: `displayName` 「研究补料」, `commands: ['sources.upsert_batch']` (shared with `cap.collect`, many-to-many), `readProfiles: ['sources', 'x_lists']`, the new optional `readToolWhitelist` field (§11), `defaultRoleBindings: { reporter: true, desk: true }` (2026-08-10 supervisor-authority flip: the supervisor's full internal standing set includes research commands), `grantKinds: { task: ['research'] }`, `precise: false`, `agentGrantable: true`, `owner: 'intelligence'`. `TASK_INTENT_NEEDED_CAPS.research = ['cap.research']`; `AUTOMATIC_TASK_GRANT_SCOPES.research = ['agent_tasks.report_progress', 'sources.upsert_batch']`; `roleForTaskIntent('research') = 'reporter'`. The reporter is the only employee role that executes research jobs; no new role is created. The first research read surface = Web + X + XHS, all read-only: Web uses the two new independent read-only tools `wmb_search_web` / `wmb_read_web_page` (zero new provider/model/platform API; they reuse existing candidate resolution and static body extraction with independent tool names, parameters and audit semantics); X and XHS reuse the already mounted read-only tools. Channel tools `wmb_resolve_intelligence_website` / `wmb_trial_intelligence_website` keep channel semantics (candidate resolution / trial read under channel audit) and never carry the research read surface.

2. **Supervisor (desk) dispatch inside the parent-task boundary.** When a fixed-role work order reports a structured `evidenceGap` (progress/checkpoint signal or an unresolved plan-item missing-materials marker), the desk — as the software supervisor — auto-dispatches one research job via `wmb_spawn_job({ roleId: 'reporter', research: { requiredClaims, budget, channels } })`. Boundary inheritance is mandatory: `businessDate` = the parent task's business day (non-empty) and `projectId` = the parent task's projectId when present. Spawn validation is fail-closed: the same `parentJobId` may have at most one active research task (a duplicate spawn returns the existing task idempotently), and the parent role must be `writer`, `planner` or `librarian` — a `reporter`/`research` parent is rejected (`VALIDATION_ERROR`). The role-job registry (`deriveRoleJobSpec`) is the sole source of intent derivation; callers must not rewrite it.

3. **ResearchGap — context contract (value object).** Persisted in the parent-visible `context_refs_json` with the shared spawn keys plus a `research` block: `jobId`, `roleId: 'reporter'`, `brief`, `businessDate`, `projectId` (when present), and `research: { gapId: 'research-<uuid>', parentJobId, parentTaskId, parentRoleId ∈ {writer, planner, librarian}, requiredClaims: [{ key, text, type ∈ {fact, price, policy} }], budget: { timeMinutes: 12, minValidSources: 15, maxCandidates: 40, maxParallelFetches: 3, maxRounds: 1 }, channels: ['web', 'x', 'xhs'] }`. `type ∈ {price, policy}` implies `needs_time_excerpt` for every evidence item. The research task's `deriveRoleJobSpec` derives `intent='research'`, `policy='research'`, `readback='research_evidence'`; `resourceLocks` = the reporter lock keys plus `research:{parentJobId}` (parallel research jobs on the same parent are mutually exclusive). This depth tier is the only tier defined by this legislation.

4. **ResearchJob — agent_tasks row.** `intent='research'` is added to the `RunnerAgentIntent` union (late-migration v54 rebuilds the agent_tasks intent CHECK; the research_claims DDL of §9 lands in the same migration). `status` reuses the existing `AgentTaskStatus` (`running` / `succeeded` / `partial` / `failed` / `cancelled` / `interrupted` / `needs_user`) with zero change. `contextRefs` = the ResearchGap contract (§3). `progress` = `{ planned: 40, processed, verified, saved, message }` — the budget counters' single persistence point. `checkpoint` = `{ round: 1, startedAt, budgetLeftMs, candidatesProcessed, claimsSnapshot: { [claimKey]: status } }` — the recoverable resume state. `resultRefs` = the EvidencePack (§6). Budgets are machine-enforced from these persisted counters, never from prompt discipline.

5. **Required-claim threshold and the ResearchClaim state machine.** `research_claims` rows move `pending → supported | contradicted | unresolved | source_unavailable` (the last four are terminal; `pending` exists only while the task runs). The reporter proposes `{ claimKey, status, evidenceSourceIds, verdictReason }` through structured output; the runner validates every proposal against the rules below before writing (system-side derivation, fail-closed; the reporter never writes the table directly):
   - `supported`: evidence reaches the support threshold — ≥1 official/first-hand source, OR ≥2 independent reliable secondary sources (independent = the two sources' canonical URL domains differ; every item carries verifiable `title` / `originalUrl` / `author` / `summary`); when `type ∈ {price, policy}`, every supporting item must additionally carry time (`publishedAt`/`collectedAt`) plus a verbatim `excerpt` of the original key sentence — missing time or excerpt fails the check.
   - `contradicted`: reverse evidence reaches the same threshold against the claim, or an official source directly overturns it.
   - `unresolved`: checked within the single round but cannot be judged (conflicting evidence below threshold, insufficient quality, or validation downgrade).
   - `source_unavailable`: every candidate source for the claim failed to read (network unreachable / dynamic render unreadable / login missing / anti-bot / captcha or login wall), leaving no evaluable material.
   A proposal that fails machine validation is downgraded to `unresolved` with reason `threshold_not_met`. Thresholds live in code, not prompts.

6. **EvidencePack and terminal states.** `result_refs_json` holds `{ kind: 'research_evidence', jobId, round, claims: [{ id, key, status, verdictReason, evidenceSourceIds, needsTimeExcerpt }], sourceIds, validSourceCount, candidateCount, timeSpentMinutes, terminalReason ∈ {claims_resolved, budget_exhausted, candidates_exhausted, aborted}, unresolvedRequiredClaims: [claimKey…] }`. Terminal mapping: `succeeded` = every required claim is terminal with none unresolved/source_unavailable; `partial` = the single round ended (budget or candidates) with at least one judged claim; `failed` / `cancelled` = runner error / user cancellation (no successor; the desk reports to the user, no auto retry); `needs_user` = blocking precondition such as missing X/XHS login (existing channel-style semantics). 15 valid sources is a quality target (recorded in the pack's quality note; its shortfall does not block a successor); 40 candidates / 12 minutes / one round are hard run boundaries.

7. **Automatic single-hop successor — the only narrow exception.** The research terminal-state handler (system-side; neither the research agent nor the desk) creates exactly one new job when the research task reaches a terminal state with an EvidencePack: `jobs` row `kind='research_successor'`, `dedupe_key='research-succ:{parentJobId}'` UNIQUE, `payload_json` = `{ parentJobId, researchTaskId, parentRoleId, unresolvedRequiredClaims, briefSuffix }`, `due_at` = now, `status` = pending. `INSERT OR IGNORE` makes handler re-entry and restart replay idempotent — at most one successor per parent job. Consumption follows the reproposal pattern: the scheduler polls due rows, `rebuildRoleJobRequest(parent contextRefs)` rebuilds the original parent-role request, appends `briefSuffix` (EvidencePack summary + the no-unsourced-numbers constraint), and the spawner derives the successor as a **new work order** (same boundary, same roleId, new jobId) — it is never a retry of the research job. Successor state gate: an EvidencePack containing unresolved/source_unavailable required claims puts the successor into `needs_user` before any Pi session runs; the card lists the unresolved claims and the three human decisions (收窄范围 / 手动补料 / 接受标注待核实); after the human acts, the desk re-dispatches it to run. A fully resolved pack (all supported/contradicted) → the successor runs directly.

8. **Hard stop-loop — three layers, any one breaks the chain.** (a) Derivation: a research job's parent role ∈ {writer, planner, librarian}; research/reporter parents are rejected fail-closed → no research→research chain exists. (b) Uniqueness: `jobs.dedupe_key = 'research-succ:{parentJobId}'` UNIQUE → exactly one auto-successor per research task, and `INSERT OR IGNORE` keeps the terminal handler re-entrant. (c) Behavior: the successor (original role) grant scope does not contain `sources.upsert_batch` (writer = `content.save_version`, planner = `plans.save`, etc.), so the original role has no research write power; if the successor again reports a material gap, the system does not auto-dispatch another research job — it turns `needs_user` for the human; reopening research requires the editor-in-chief's explicit decision (manual orchestration, never an automatic loop).

9. **Persistence and the one new table.** ResearchGap + budget + required-claim thresholds + parent/research/successor chain live in `context_refs_json`; budget counters in `progress_json`; claim state + resume scene in `checkpoint_json` + `research_claims`; EvidencePack in `result_refs_json`; needs_user/cancel reuse the existing state machine; the one auto-successor lives in the `jobs` table as `kind='research_successor'`. The **only new table** is `research_claims` — the research evidence verdict table (query/audit surface; a business fact table, not a successor-contract table):
   ```sql
   ...
   ```
   This is the sole exception to CAP-027 §8's no-new-table rule (see §8 there); it does not conflict with PRODUCT C9.10, whose no-new-table/column commitment covers authorization persistence and the successor contract only. Restart: research joins `recoverInterruptedAgentTasks`'s resume_pending set (running → `phase='resume_pending'`), resuming from `checkpoint_json` + `research_claims` within the remaining budget. Cancel: existing `cancelAgentTask`; committed evidence is retained; no successor is triggered. Idempotency: research spawn dedupes by `parentJobId`; `wmb_save_source` requestId = `agentRequestId(taskId, 'source:<n>')` deterministic replay; claim writes upsert by `UNIQUE(task_id, claim_key)`; the successor is deduped by `dedupe_key`.

10. **Write-back intersection (precise semantics).** `wmb_save_source` → `sources.upsert_batch` is a business write (writes the current root's source store), not a platform side effect. `cap.research` declaring `precise: false` means only that no Owner-UI precise side-effect confirmation is required; it does not bypass the existing formula — every write must still pass the exact intersection of automatic task grant (scope contains the command), role capability (`cap.collect` ∪ `cap.research`), execution/task envelope (`taskId` / `grantId` / `workerLeaseId` / `requestId`) and the object-boundary assertion (research write-back items carry no `feedId` → boundary claim empty → pass; research evidence must never attach to a channel feed). Missing envelope fields or a non-empty `feedId` → boundary assertion rejects with zero write. Field contract: `title` / `originalUrl` / `summary` / `author` required; `originalUrl` is the canonical dedupe key (canonicalized before reference, per WMB-4916); optional `publishedAt` / `excerpt` / `clientLabel` (not required outside research tasks); `clientLabel = 'WMB research'` and categories marked 「研究补料」 for origin traceability. Price/policy claim evidence must carry `publishedAt` + `excerpt`, or claim validation fails (§5).

11. **Read whitelist and the hard read gate (narrow first landing).** Research sessions are fail-closed to this tool set: infrastructure `wmb_get_agent_task` / `wmb_report_agent_progress`; Web read `wmb_search_web` / `wmb_read_web_page`; X read `wmb_read_x_list_index` / `wmb_read_x_list_detail` / `wmb_read_x_list_members` / `wmb_read_x_list_timeline` (≤50 items per read); XHS read `xhs_check_login_status` / `xhs_search_feeds` / `xhs_get_feed_detail` / `xhs_user_profile`; internal dedupe reads `wmb_get_source` / `wmb_search_sources`; the single write `wmb_save_source`. `readToolWhitelist` is a new optional `AgentCapability` field (absent = no constraint, so every other role/intent has zero regression); the read projection is `roleReadTools(role) = ∪ readToolWhitelist(enabledCaps(role))`. The MCP read-tool dispatch entry must enforce this projection by `taskId → agent_task.intent` and return `READ_PROFILE_BLOCKED` (reason `RESEARCH_READ_WHITELIST`) plus a `role_authority_blocked` audit row for any out-of-whitelist call — including channel `wmb_resolve_intelligence_website` / `wmb_trial_intelligence_website`, `wmb_get_workbench`, `wmb_get_content`, `wmb_save_plan`, `wmb_record_knowledge`, `wmb_judge_sources`, x_lists prepare/execute and `wmb_spawn_job`. The research prompt states the whitelist as discipline only; the dispatch layer is authoritative. This is the narrow-domain first landing of the read gate (role-permission design P1.4): it constrains research intent only, everything else unchanged.

12. **Context discipline.** `wmb_get_workbench` is forbidden in research sessions (context blowout); the runner assembles the research brief (parent boundary + requiredClaims + existing-evidence summary) in the daily-brief pattern. Research candidates/evidence are never injected wholesale into the parent task's context; the parent consumes only the EvidencePack (result_refs summary + sourceIds) and reads evidence bodies on demand.

13. **UI projection.** Research tasks render as reporter cards (role grouping unchanged): phase 「研究中」, progress `processed/planned` and `verified/15`, claim-verdict counts (supported / contradicted / unresolved / source_unavailable), start time. Terminal states exit the active view; a `needs_user` card stays under 「等你批」 holding no slot/lease/grant/lock, listing the unresolved required claims and the three actions (收窄 / 手动补料 / 接受标注待核实). Projection reuses the existing `crew-instance-projection` (needs_user dedupe, jobId anchor, durable rebuild after restart); research jobs travel the same JobPool pipe as every other employee job. Today: only the successor card with unresolved required claims surfaces — 「等你批 — <claim> 未解决」 plus the three decision actions; research progress, candidate lists, run logs and raw material never reach Today (C2/C4). The desk observes research tasks via the existing `JOB_EVENT` terminal push + `wmb_get_job` (never sleep/bash polling); `wmb_spawn_job` gains the research parameter surface (roleId='reporter' + research block), the desktop tool surface otherwise unchanged; `wmb_search_web` / `wmb_read_web_page` belong to the research read surface only.

14. **Web security and X/XHS/publish red lines.** `wmb_search_web` reuses `resolveWebsiteCandidates` (Bing candidate resolution + URL normalization). `wmb_read_web_page`: static body extraction first; on static failure a controlled headless-browser fallback renders dynamic public pages (render-only, no user-script interaction, no cookie/session injection, no second-platform login state). Captcha/login walls are never bypassed — explicit failure `source_unavailable` (reason `auth_required`), no credential carrying. URL/resource limits shared by static and fallback, fail-closed: `assertPublicUrl` SSRF guard (reject private / loopback / internal / link-local addresses) + DNS-rebinding protection (post-resolution target-IP re-check) + restricted redirects (http/https only, per-hop host re-validation) + size cap (body ≤ 2 MiB) + type whitelist (document types such as text/html) + timeout cap (default 15s); all denials record audit rows. X red lines: research uses only the read-only `read_*` tools; `wmb_prepare_x_list_operation` / `x_lists.operation_execute` stay REDLINE_COMMANDS (Precise + Owner UI) and outside research scope; research never starts/stops observations (`observation_start/stop` not in the research scope). XHS red lines: `XHS_FORBIDDEN_TOOLS` (publish / comment / like / favorite / cookie delete / QR-code login) are never exposed; research allows only the four XHS read tools. Publish/hard-delete unchanged: the final publish click and hard-delete remain human-UI-only and unreachable by any instance/grant combination.
### CAP-029 Zhihu hot question driven daily article×video-script loop (M-5330 foundation)

Links: REQ-032, AC-030, PRD §2.6.
Normative companion: `docs/spark/2026-08-22-zhihu-hot-question-content-loop-PLAN.md` (正式 Owner lock 2026-08-22; **WMB-5330 只做共享基础，不实现浏览器/Today/Studio/调度**). Uses `src/shared/daily-content-loop.ts` as command/type single source.

1. **Schema (v75, SQLite sole truth).** Five tables, active-root dispatcher is sole write path; no publish/second workbench/service/database/identity changes:
   - `zhihu_hot_observations(id, source_item_id→source_items, business_date, rank 1..100, heat_text, question_title_snapshot, question_url_snapshot, excerpt_snapshot, evidence_url, collected_at, scan_task_id, input_fingerprint, created_at, UNIQUE(source_item_id,business_date,input_fingerprint))` + indexes on `(business_date,rank)` and `(source_item_id,business_date)`.
   - `daily_content_cycles(id, business_date UNIQUE, timezone, target_count 1..5 default 2, status pending|running|needs_user|completed|partial|paused|failed, plan_id→plans, started_at, completed_at, last_error_code, created_at, updated_at, revision>=1)`.
   - `daily_content_targets(id, cycle_id→daily_content_cycles, target_kind new_content|draft_revision|published_revision, counts_toward_goal 0|1, source_item_id→source_items, plan_item_id→plan_items, project_id→content_projects, predecessor_content_version_id→content_versions, predecessor_publication_id→publications, predecessor_target_id→self, carry_depth 0|1, selection_mode automatic|owner_approved|carried, score_snapshot_json frozen, format_decision_json, status proposed|selected|researching|drafting|article_ready|scripting|completed|blocked|skipped|carried, blocked_reason_code, revision>=1, CHECK kind/counts lineage, partial unique indexes `cycle+source` for new_content and `cycle+predecessor_version/publication` for revision targets)`.
   - `content_derivatives(id, project_id→content_projects, kind video_script, UNIQUE(project_id,kind))`.
   - `content_derivative_versions(id, derivative_id→content_derivatives, source_content_version_id→content_versions, version_number>=1, format_decision_json frozen, title, body, status draft|ready, author ai|user, UNIQUE(derivative_id,version_number), BEFORE UPDATE/DELETE triggers `CONTENT_DERIVATIVE_VERSION_IMMUTABLE`)`.
   Migration runs from empty and legacy v74; preserves data/indexes/triggers/FKs; does not rebuild unrelated tables.

2. **Frozen command/type contracts (no handlers).** Commands are declaration-only; runtime registration left to owning tasks WMB-5331..5337. Single source `src/shared/daily-content-loop.ts`:
   `intelligence.zhihu_hot.scan`, `daily_content_cycle.ensure|pause|resume`, `daily_content_target.select|replace|skip|carry|transition`, `content_derivative.ensure|save_version|finalize_version`; types `ScoreSnapshot(6 dims+frozen JSON, risks)`, `FormatDecision`, cycle/target/derivative statuses and `DAILY_CONTENT_LOOP_COMMANDS` tuple. `finalize_version` is append-only ready version (not update).

3. **Least-privilege role grants.** New capabilities (see `src/shared/agent-capabilities.ts`):
   - `cap.zhihu_hot_collect(reporter,desk)` → `intelligence.zhihu_hot.scan` (Reporter scan/observation);
   - `cap.daily_content_cycle(planner,desk)` → cycle/target commands (Planner cycle/target decisions);
   - `cap.content_derivative(writer,desk)` → derivative `ensure|save_version|finalize_version` (Writer derivative versions);
   - Desk union = above three + `INFRA_GRANT_COMMANDS`, no publish/redline grants. `filterCommandsForRole` enforces intersection; WORKSPACE_BUSY/GRANT_REQUIRED still dispatcher-enforced.

4. **Invariants frozen for later slices.** Stable keys: `observation=source+businessDate+fingerprint`, `cycle=workspace+date`, `new_target=cycle+source+kind`, `revision_target=cycle+predecessor`, `derivative=project+kind`, `derivative_version=derivative+versionNumber` (requestId replay). `carry_depth≤1` prevents infinite debt; `completed` requires `article_ready + ready script`; `counts_toward_goal` matches `kind`. All reads/writes via `CommandEnvelopeV1` + `CommandDispatcher`/`dispatchBusinessCommand` on the active `wmb.db` connection with `workspaceId+runtimeEpoch` validation, inputHash replay, revision conflict and audit; data-root isolated; swallowed errors replay as dispatcher receipts.
...
| EVAL-030 | Crew multi-instance runtime | Same-role multi-instance visibility (two reporter jobs on different channels render 「记者 #1」/「记者 #2」 cards with independent progress). Zero-spawn empty state (five role groups always visible, each 「当前无任务」, header 工作中 0 · 排队 0, no placeholder seats or 「待命」). Terminal exit vs needs_user stay (succeeded/failed/cancelled leave the active view and stay identifiable by jobId; needs_user holds no slot/lease/grant/lock, does not auto-retry, desk reports 「需要你 X」, human action re-dispatches or closes; scan→judge keeps one active instance per task without double count). Capacity semantics (maxWorkers=2: three cross-role jobs → 2 running + 1 queued FIFO; same-role parallel running; maxWorkers=0 rejects spawn; capacity change touches no role/registry). Desk boundary (full internal standing write power = all grantable business-capability commands ∪ INFRA including internal prepare commands, never employee slot, `spawn(roleId:'desk')` rejected, internal approval belongs to the supervisor, three redline categories — final publish / hard-delete execution / external platform change execution — are human-UI fresh-confirmed only, single-hop dispatch only; 2026-08-10 supervisor-authority flip). Object gate (cross-object write BLOCKED + audit; publish/hard-delete/platform side effects unreachable for every instance/grant combination). Restart re-dispatch from `context_refs_json` (jobId/roleId/brief/boundary params; no new table/column; one-click rebuild of RoleJobRequest). Construction change set leaves the three-table schema, Capability registry and role set unchanged with `check:capabilities` green. |
| EVAL-031 | Topic maintenance approval | A librarian job creates an exact duplicate-topic merge proposal without changing formal topics. Reject leaves all topic/link revisions unchanged. Approve migrates source/plan/content/review links and archives the old topic in one transaction with complete receipt/readback. Approval consumes the frozen v2 conflict contract: unrelated graph drift passes, while target revision/canonical/migration-membership conflicts atomically produce structured stale + one durable reproposal outbox and zero formal writes. Commit occurs before Agent dispatch; duplicate kick/click and cold restart yield one librarian successor linked to the immutable stale parent, and only a new supervisor (desk) approval can apply it (2026-08-10 supervisor-authority flip: topic approval is supervisor-internal). Historical stale rows remain unclassified and are not revived. Librarian/planner/external-Agent direct apply attempts are blocked; the supervisor applies and reads back. Topic shows pending/reproposing/superseded history, Today shows only pending decisions, and the supervisor never asks the Owner to edit manually. |
| EVAL-032 | ResearchJob evidence supplement and single-hop successor | GLM 5.2 anomalous-low-price fixture (@AbionMorse post + official pricing page + OpenRouter model page + one dynamic-render fallback fixture) runs end-to-end with all 13 checks: (1) a writer job (projectId=P) reports `evidenceGap` (requiredClaims incl. `glm52_official_price_rise`, type=price) → the desk auto-spawns a research job in the same boundary (businessDate + projectId=P), and the same parentJobId has at most one active research task. (2) The research task renders as a reporter card with real progress (planned=40, verified target 15). (3) Read gate: out-of-whitelist tools (`wmb_get_workbench`, channel `wmb_trial_intelligence_website`) → `READ_PROFILE_BLOCKED` (reason RESEARCH_READ_WHITELIST) + audit row; in-whitelist search_web/read_web_page/X/XHS reads work. (4) Write-back: every `wmb_save_source` carries canonical originalUrl (re-import adds no new source); price-claim evidence carries publishedAt + excerpt; no feedId; missing taskId/grantId/workerLeaseId/requestId or a feedId → boundary assertion rejects with zero write (precise:false exempts only Owner-UI confirmation). (5) Claim machine validation: forged supported with below-threshold evidence (1 secondary / missing time or excerpt) → downgraded unresolved (threshold_not_met); official price page 1 first-hand + independent secondary → supported; official page showing no rise → contradicted; all candidates unreadable → source_unavailable. (6) Single round: candidates exhausted or 12 minutes elapsed → terminal partial + EvidencePack (round=1). (7) Auto successor: research terminal → `jobs` row kind='research_successor' with dedupe_key='research-succ:{parentJobId}'; replaying the terminal handler for the same parentJobId still yields exactly one successor; the successor is a writer job (same projectId) whose brief appends the EvidencePack summary. (8) Hard stop-loop: the successor writer again reports a gap → no automatic research re-dispatch → needs_user card; spawning a research job whose parent intent is research → VALIDATION_ERROR. (9) Today projection: an unresolved-required-claims successor → exactly one 「等你批」 card with 收窄/手动补料/接受标注待核实; research progress and raw material never appear on Today. (10) Restart recovery: a running research task → resume_pending, resumes from checkpoint + research_claims within the remaining budget; an enqueued un-consumed successor is consumed exactly once after restart. (11) Artifact quality: the successor's new draft has no unsourced numbers and no wholesale copy of the old draft; 「official price rise」 moves from unverified to supported/contradicted or is narrowed/labelled per user decision. (12) Cancel/failure: cancelling research retains committed evidence and triggers no successor; failed research → desk reports to the user, no auto retry. (13) Web security: static read failure → fallback renders the dynamic public fixture and returns its body; the same URL behind a captcha/login wall → explicit failure (reason auth_required) with no bypass and no session credential; private/loopback URL → SSRF rejected; DNS-rebinding host → rejected; redirect leaving the whitelisted domain → rejected; >2 MiB page / non-document type / >15s timeout → rejected or truncated — each denial recorded in the audit. |
