...
Settings 中的 X Lists 是所有自媒体工作空间固定具备的列表管理与情报接入配置，不是通用 X 客户端；Discover 只浏览用户选择显示的 List 动态。模块从当前 WMB 安装共享的专用 Edge 登录态实时读取三类可访问 List：用户创建、用户关注、用户在其中。稳定身份是 workspace ID、当前安装账号、X List ID 与规范 URL；名称不是身份。WMB 只在当前根持久化账号快照、用户绑定为情报信源的 List 及每次操作的证据，不镜像全部成员或完整动态历史。

X Lists 是通用情报来源能力，不由 `WorkspaceProfileV1.platforms` 启停，也不因工作空间未选择 X 作为发布平台而消失。一套 WMB 安装只保存并使用一个专用 Edge profile、浏览器配置和当前 X 登录态，不要求用户为每个工作空间重复登录或使用指纹浏览器。每个根仍独立保存该账号的校验快照、List 绑定、缓存、操作记录和收集资料；不得继承、转发或命中其他根的 binding、cache、operation 或 source feed。既有 `AI前沿` binding 转为 AI 根中可见、可管理的普通来源配置；UK、游戏资讯或其他赛道使用相同模块但只能读取本根用户明确启用的 List。
...
每个外部写入先读取当前账号、List 身份/所有者、权限和必要的页面快照，生成冻结的变更集。成员添加或移除是例外：用户在当前 Pi 对话中明确指定目标 List、精确账号和加入/移出意图后，Pi 必须先确定唯一 `@handle`，再调用 `x_lists.members_add` 或 `x_lists.members_remove`；该 MCP 调用本身即为执行入口，不再生成第二次 UI 确认。命令必须绑定 workspace、当前账号、List ID、request ID 和完整 handles，逐项点击前从结果 profile URL 精确验证 handle，相似显示名或文字包含不得命中。创建、编辑和删除 List 仍由应用级 UI 最终确认；删除还要求输入当前 List 名称。所有路径使用同一 operation 保存进度、停止能力和逐项读回。
...
通用 `x_lists.*` UI/IPC、MCP 和 Pi 能力使用同一业务命令，在每个有效自媒体工作空间可用，并在每次调用时重新验证当前 workspace、data-root、安装级当前账号和本根 List 绑定。缺少登录、当前账号与本根绑定快照不匹配、挑战或权限不足进入准确的 `needs_user`；账号不匹配的稳定 reason/error code 为 `ACCOUNT_MISMATCH`，不得迁移旧账号 binding、复用旧账号 cache 或静默回退。MCP/Pi 不暴露确认工具；`members_add` 和 `members_remove` 在用户明确指令下直接执行，其余外部写入仍由 UI 精确确认。
...
Exactly one workspace runtime may be active. SQLite, MCP, Pi, Xiaohongshu MCP, scheduler, jobs and renderer reads are bound to that root. The managed Edge process and its CDP connection are also stopped on a switch, but its installation-level profile and login state persist and are reused after relaunch. A switch is rejected with `WORKSPACE_BUSY` while a Pi/content/intelligence/review task is running, an external browser write cannot be safely interrupted, or the current mutation queue cannot be drained. An allowed switch closes the mutation gate, rejects new mutations, waits for committed database/file writes, validates the target root and identity read-only, records `previous_workspace_id + pending_workspace_id`, stops every root-bound runtime and browser process, closes the database and MCP server, then relaunches the Electron application. It does not rebind a live renderer to another root.
...
The platform subset controls new plan platform selections, platform versions, publishing actions and platform-specific runtimes. It does not hide historical records or control shared intelligence inputs. Website and X Lists modules remain fixed WMB capabilities rather than profile fields or plugins; website configuration and all X business state remain root-local while the dedicated Edge login is installation-level. Every intelligence pack may consume only sources explicitly enabled in the current root.
...
| EVAL-016 | X List workspace | AI、UK、游戏资讯三个真实根的 MCP `tools/list` 均包含不带确认权限的通用 `x_lists.*`；a valid profile fixture whose publishing platforms exclude X still exposes X Lists; after one login, each active workspace uses the same installation-level dedicated Edge profile to read the current account's owned/followed/member List identities without another login. Explicit exact-handle member add/remove batches are frozen, directly executed through their dedicated MCP tools, serially read back and stoppable; create/update/delete remain UI-confirmed. Changing root/account/profile before execution rejects the operation with zero platform/business write. Account A → B invalidates A-bound root state with `ACCOUNT_MISMATCH`, no binding migration or cache reuse. A bound List timeline writes traceable current-root source items; identical account/List/URL/cache/source-feed fixtures remain mutually invisible across roots; unsafe external-list actions remain absent. |
...
| EVAL-019 | Official profiles and new lanes | Without a configured model, UI creates the fixed UK template; UK completes a linked source → plan → content → X pure-text platform version through UI/Pi/external MCP while every inventoried genuinely AI-only route stays at zero. AI positively retains rankings, while AI/UK/game all expose the same website/X Lists channel surface, reuse one installation-level Edge login, and never share source configuration, bindings, cache, receipts or items. Studio, Publish and Results show only enabled new actions; UI, IPC, MCP and platform runtimes reject new work outside the current publishing-platform subset with zero write while preserving historical readback. A non-self-media proposal and each stale confirmation binding are rejected with zero registry/root/profile change. 游戏资讯根 is confirmed manually in the real UI, cold-reopened, and completes the same linked text chain; interrupted candidate-root initialization leaves registry/active unchanged and is safely revalidated. Missing model or installation-level X login produces one persistent, accurately attributed `needs_user` with stable reason/workspace/profile revision, no duplicate unchanged attempt, fallback or cross-root write. |
