...
9. **Shared pool is capacity, not quota; desk is a coordination entry, not a supervisor workstation; red lines unchanged**: `maxWorkers` (0..7, 0 disables dispatch, default 2) is the all-role shared concurrency cap and must not be read as per-role staffing or quota; the human editor-in-chief is the supervisor, desk has no standing write power, never occupies an employee slot and never approves on the human's behalf; final platform publish, hard-delete and platform side effects remain human-UI-only (`agentGrantable: false`) for every instance and grant combination. Instance first-class identity is `jobId`; display numbering is active-period-only; the spawn contract persists jobId/roleId/brief/boundary params in the existing `context_refs_json` for restart re-dispatch. See CAP-027.
10. **Librarian topic maintenance is proposal-first**: Agents may prepare exact topic change sets, but formal topic/domain structure and linked-object relations change only after Owner UI approval. Desk presents the proposal and never substitutes manual Owner editing for employee work. See CAP-003 and `docs/spark/2026-08-10-topic-maintenance-approval-design.md`.
...
9. `librarian` owns topic-housekeeping proposals (create/update/merge/archive/reassign) but never applies them. Topic proposal application/rejection is Owner-UI-only and must not become standing role power; planner keeps topic/opportunity judgment without bypassing the same approval boundary for formal topic-structure changes.
...
6. **Desk = coordination entry, not supervisor workstation.** The human editor-in-chief is the supervisor. Desk has no standing write power, never occupies an employee slot and must never approve on the human's behalf (`spawn(roleId:'desk')` → `ROLE_NOT_SPAWNABLE`); its tools are orchestration/read-only (roster/jobs/message), not business write commands, and its answers come only from projection APIs. Dispatch is single-hop: the desk hands one bounded job to one instance, and instances must not re-dispatch to each other (no orchestration graph, no auto multi-hop).
