...
syncedAt: "2026-08-12T09:36:40.104Z"
...
typography:
  display:
    fontFamily: "\"Inter\", \"PingFang SC\", \"Microsoft YaHei UI\", \"Segoe UI\", system-ui, sans-serif"
    fontSize: "34px"
    fontWeight: 700
    lineHeight: 1.16
...
>
> Last sync: `2026-08-12T09:36:40.104Z`
...
foundation:
  dark:
    ...
    topbar-height: "56px"
    page-space: "16px"
    font-sans: "\"Inter\", \"PingFang SC\", \"Microsoft YaHei UI\", \"Segoe UI\", system-ui, sans-serif"
  ...
  light:
    ...
    surface-hover: "#f0ecfa"
    surface-selected: "#ebe6f8"
    tag-bg: "#eeeaf7"
    ink: "#221e2c"
    ink-soft: "#3e384a"
    muted: "#625b70"
...
黑夜紫罗兰是默认主题，适合长时间专注工作；白昼紫罗兰使用同一语义 token（设计变量）映射到明亮表面，不改变信息层级。正式 Logo 唯一来源是 `images/logo.png`，使用时裁切 W+眼睛符号或横向字标，不得重绘变体；真正的 SVG 母版交付后替换，不改变布局占位。Owner 批准 WMB 小动物作为品牌角色，完整动作图鉴位于 `docs/design/brand-motion/wmb-creature-motion-library.html`，角色语汇是“机灵、好奇、克制的小主编”。走路母版仍以 `docs/design/brand-motion/wmb-creature-walk.html` 为唯一权威版本：白色身体保持一条连续复合路径，两个底部尖角通过柔性路径形变交替承重，位移与重心起伏锁定同一步态相位，眨眼使用独立自然周期；具体页面只使用与用户状态一致的微动作，不得无目的滥用场景动作。
...
- 中部按发生顺序在同一个 Pi 回复气泡内连续显示可见思考、正式回复和工具进度。最新的进行中思考保持展开流式显示；已经过去的思考仍留在原顺序位置，但默认收为一行摘要并可主动展开全文，避免模型反复推演淹没正式内容。工具调用默认只显示一行 `工具名 · 当前任务` 摘要；调用参数与原始结果仅在用户主动展开该行后显示，不得默认倾倒完整调用链。
- 底部使用单一输入区和发送按钮。
