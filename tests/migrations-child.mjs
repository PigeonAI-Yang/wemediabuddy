...
assert.equal(first.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get().count,35);
first.close();
const second=migrateDatabase(databasePath);
assert.equal(second.prepare('SELECT COUNT(*) AS count FROM schema_migrations').get().count,35);
