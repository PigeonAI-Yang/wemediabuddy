...
import {
  IntelligenceChannelProposalStore,
  channelProposalBinding,
} from "../src/main/intelligence-channel-proposals.ts";
...
function actor(lane = "agent-work-path") {
  return {
    actor: { type: "scheduler", id: lane, label: lane },
...
async function startTask(runtime, input, workerLeaseId) {
  ...
  return dispatchStartAgentTask(
    runtime,
    { ...input, contextRefs },
    { ...actor("start"), workerLeaseId, taskId: undefined },
  );
}

async function progress(runtime, taskId, body, workerLeaseId) {
  return dispatchReportAgentTaskProgress(runtime, taskId, body, {
    ...actor("progress"),
    taskId,
    workerLeaseId,
  });
...
async function cancelTask(runtime, taskId, workerLeaseId) {
  return dispatchCancelAgentTask(runtime, taskId, {
    ...actor("cancel"),
    taskId,
    workerLeaseId,
  });
...
test("0 catalog has five agents with rooms", () => {
  assert.deepEqual(Object.keys(ROLE_CATALOG).sort(), [
    "desk",
    "librarian",
    "planner",
    "reporter",
...
test("1 desk: exclusive seat, page_today start/cancel, full standing grant (A5)", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      [...commands].sort(),
      deskStandingCommands(),
      "desk page grant covers full standing",
    );
...
test("2 reporter: daily_scan grant collect-only, progress, not desk seat", async () => {
  ...
  try {
    ...
    const { commands } = await grantCommands(
      runtime,
      started.task.id,
      "reporter",
    );
    ...
    assert.equal(
      roster.find((r) => r.roleId === "reporter")?.status,
      "running",
    );
  ...
  try {
    ...
    const rebound = await dispatchUpdateAgentTaskPhase(
      ...
      },
      {
        ...actor("rebind"),
        taskId: scan.task.id,
        workerLeaseId: scanLease.leaseId,
      },
    );
    ...
    assert.equal(
      getAgentTask(runtime.database, scan.task.id).phase,
...
test("4 writer: studio_draft on project, grant write-only", async () => {
  ...
  seedDb
    ...
    .run("workspace_id", `ws-${path.basename(directory)}`, now, now);
  const projectId = createContentProject(seedDb, {
    title: "验收用草稿项目",
  }).id;
  seedDb.close();
  ...
  try {
    ...
    await progress(
      runtime,
      started.task.id,
      { phase: "running_pi", message: "撰写初稿中" },
      lease.leaseId,
    );
    assert.equal(
      getAgentTask(runtime.database, started.task.id).phase,
      "running_pi",
    );
  ...
  try {
    ...
    try {
      ...
      await progress(
        runtime,
        started.task.id,
        { phase: "running_pi", message: "整理资料库" },
        lease.leaseId,
      );
      ...
      librarianRunning =
        getAgentTask(runtime.database, started.task.id)?.status === "running";
    }
    ...
    if (librarianRunning)
      assert.equal(
        roster.find((r) => r.roleId === "librarian")?.status,
        "running",
      );
    assert.notEqual(roster.find((r) => r.roleId === "desk")?.status, "running");
    ...
    const spawner = new JobSpawner(runtime, {
      ...
      execute: async ({ job, lease, onTaskBound }) => {
        ...
        const started = await dispatchStartAgentTask(
          runtime,
          {
            intent: "page_library",
          ...
          },
          {
            ...
            requestId: `t:${job.id}:start`,
          },
        );
        runtime.bindWorkerTask(lease, started.task.id);
        onTaskBound?.(started.task.id, null);
        assert.equal(
          spawner.getHandle(job.id)?.taskId,
          started.task.id,
          "taskId 经 onTaskBound 回写 handle",
        );
        return {
          status: "succeeded",
          code: "OK",
...
test("7 permission matrix foreign writes blocked", () => {
  ...
  assert.deepEqual(
    [...roleWriteCommands("desk")].sort(),
    deskStandingCommands(),
    "desk standing full while employees stay page∩role",
  );
});
  ...
  try {
    ...
    const spawner = new JobSpawner(runtime, {
      ...
      execute: async ({ job, lease, onTaskBound }) => {
        ...
        const snaps = runtime
          .getWorkerSnapshots()
          .filter((s) => s.purpose === "employee");
        assert.ok(snaps.some((s) => s.leaseId === lease.leaseId));
        assert.equal(
          runtime.getWorkerSnapshot()?.purpose === "employee",
          false,
        );
        return {
          status: "succeeded",
    ...
    const done = await Promise.all(
      jobs.map((j) => spawner.await(j.id, 20_000)),
    );
    assert.ok(
      done.every((d) => d.status === "succeeded"),
      done.map((d) => `${d.status}:${d.error}`).join(" | "),
    );
...
test("9 getLatestDailyIntelligenceTask prefers succeeded over later cancelled", async () => {
  const { getLatestDailyIntelligenceTask } =
  ...
  try {
    ...
    database
      ...
      progress_json, checkpoint_json, events_json, control_action, heartbeat_at, error_code, error_message,
      created_at, updated_at, finished_at
    ) VALUES (?,?,?,?,?,null,'{}','{}','{}','{}','[]',null,null,'CANCELLED','用户取消',?,?,?)`,
      )
      .run(
        "cancel-task",
        "daily_judge",
...
test("10 A2 supervisor cross-page full grant; employee publish page stays readonly", async () => {
  ...
  try {
    ...
    const published = await startTask(
      runtime,
      {
        intent: "page_publish",
        businessDate: "2026-08-08",
    ...
    assert.deepEqual(
      [...commands].sort(),
      deskStandingCommands(),
      "publish page desk grant = full standing",
    );
    ...
    assert.ok(
      agentsGrant.commands.includes("plans.save"),
      "desk grant not narrowed by page scope",
    );
    assert.ok(
      agentsGrant.commands.includes("knowledge.topic_maintenance_approve"),
    );
...
test("11 A6 desk stale-scope write refreshes exactly once, replay succeeds (no loop, no duplicate write)", async () => {
  ...
  try {
    ...
    assert.equal(
      handlerCalls,
      1,
      "exactly one business write — no recursive retry, no duplicate write",
    );
    ...
    assert.equal(
      oldGrant.status,
      "revoked",
      "old narrow grant must be revoked by sameCommandSet renewal",
    );
    const active = listTaskGrants(
      runtime.database,
      started.task.id,
      new Date(),
      runtime.identity,
    ...
    assert.deepEqual(
      [...active.allowedCommands].sort(),
      deskStandingCommands(),
      "renewed grant = deskStanding",
    );
    ...
    assert.equal(
      handlerCalls,
      1,
      "idempotent replay must not re-run the handler",
    );
...
test("12 A6 redline and employee writes never trigger the stale-scope refresh", async () => {
  ...
  try {
    ...
    assert.equal(narrow.ok, true);
    const redline = await runtime.dispatchCommand(
    ...
    assert.equal(
      ...
      "redline never enters any grant",
    );
    assert.equal(
      getTaskGrant(runtime.database, narrow.data.id).status,
      "active",
      "redline must not trigger renewal",
    );
    ...
    const plannerLease = runtime.acquireWorkerLease(
      null,
      "planner",
      "employee",
    );
    const plannerTask = await startTask(
      runtime,
      {
        intent: "page_today",
        businessDate: "2026-08-08",
    ...
    const { grantId } = await grantCommands(
      runtime,
      plannerTask.task.id,
      "planner",
    );
    const employeeWrite = await runtime.dispatchCommand(
      createCommandEnvelope({
        workspaceId: runtime.identity.workspaceId,
        runtimeEpoch: runtime.identity.runtimeEpoch,
    ...
    assert.equal(
      employeeWrite.error.code,
      "TASK_SCOPE_BROADENED",
      "employee out-of-role write stays rejected",
    );
    assert.equal(
      listTaskGrants(
        runtime.database,
...
test("13 WMB-5183 channel safe-apply: desk applies add/enable/disable with readback; remove proposal denied pre-mutation", async () => {
  ...
  try {
    ...
    assert.ok(
      grant.allowedCommands.includes(
        "intelligence_channels.proposal_apply_safe",
      ),
      "safe-apply is desk standing",
    );
    assert.equal(
      grant.allowedCommands.includes("intelligence_channels.proposal_apply"),
      false,
      "precise apply never enters any grant",
    );
    ...
    const receipt = await dispatchConfirmIntelligenceChannelProposalSafe(
      runtime,
      {
        store,
    ...
    assert.deepEqual(receipt.readback, {
    ...
    assert.equal(
      runtime.database
        .prepare("SELECT COUNT(*) AS count FROM website_sources")
        .get().count,
      1,
    );
    assert.equal(
      receipt.executionGrantId,
      null,
      "safe-apply needs no precise execution grant",
    );
    ...
    const added = summary.sources.find(
      (source) => source.module === "official_web",
    );
    ...
    const before = runtime.database
      .prepare("SELECT COUNT(*) AS count FROM website_sources")
      .get().count;
    const denied = await dispatchConfirmIntelligenceChannelProposalSafe(
      runtime,
      {
        store: removeStore,
    ...
    assert.equal(
      runtime.database
        ...
        .get().count,
      before,
      "remove proposal must not mutate",
    );
    assert.equal(
      runtime.database
        .prepare("SELECT COUNT(*) AS count FROM execution_grants")
        .get().count,
      0,
