...
test('empty-run no-op: zero new sources → gate no-op zero writes, empty plan saves, task completes succeeded (AC-017)', async () => {
  await withRuntime(async ({ root, runtime, database }) => {
    const planDate = shanghaiDate();
    // 一个启用的 X List 来源（渠道就绪），但本轮扫描零候选零保存 → 零新入库。
    const envelope = createCommandEnvelope({
      workspaceId: runtime.identity.workspaceId,
      runtimeEpoch: runtime.identity.runtimeEpoch,
      command: 'test.lane.seed_empty_run',
      requestId: `seed-empty-${randomUUID()}`,
      input: {},
      boundIdentity: { entityType: 'source_item' },
      actor: owner
    });
    const receipt = await runtime.dispatchCommand(envelope, () => {
      const database = runtime.database;
      const feed = createSourceFeed(database, { name: 'X List · AI前沿' });
      ...
      database.prepare(`INSERT INTO x_list_bindings
        (id, account_key, list_id, canonical_url, owner_handle, name, list_kind, source_feed_id, enabled,
         last_observed_at, last_observation_json, created_at, updated_at, revision)
        VALUES (?,?,?,?,?,?,?,?,1,?,?,?,?,1)`).run(
        bindingId, 'acct-a', AI_FRONTIER_LIST_ID, `https://x.com/i/lists/${AI_FRONTIER_LIST_ID}`,
        'owner-a', 'AI前沿', 'owned', feed.id, now, '{}', now, now
      );
      return { data: { bindingId, feedId: feed.id }, entityType: 'source_item', entityId: bindingId };
    });
    assert.equal(receipt.ok, true);
    const { bindingId, feedId } = receipt.data;
    const now = new Date().toISOString();
    ...
    const frozen = {
      ...
      profileRevision: 1,
      modules: ['x_lists'],
      sources: [{ module: 'x_lists', sourceId: bindingId, sourceFeedId: feedId, revision: 1, accountKey: 'acct-a', listId: AI_FRONTIER_LIST_ID }]
    };
    const started = (await dispatchStartAgentTask(runtime, {
      intent: 'daily_intelligence', businessDate: planDate,
      contextRefs: { planDate, workspaceId, workspaceProfileRevision: 1, intelligenceChannels: frozen }
    }, { actor: owner, requestId: `task-${randomUUID()}` })).task;
    // 渠道检查回执：0 候选 0 保存（零更新）。
    const receiptEnvelope = createCommandEnvelope({
      workspaceId: runtime.identity.workspaceId,
      runtimeEpoch: runtime.identity.runtimeEpoch,
      command: 'test.lane.record_empty_receipt',
      requestId: `receipt-${randomUUID()}`,
      input: { taskId: started.id, bindingId, feedId },
      boundIdentity: { entityType: 'source_scan_receipt' },
      actor: owner
    });
    await runtime.dispatchCommand(receiptEnvelope, () => {
      recordSourceScanReceipt(runtime.database, {
        taskId: started.id, workspaceId, module: 'x_lists', sourceId: bindingId, sourceFeedId: feedId,
        status: 'succeeded', candidateCount: 0, savedCount: 0, checkedAt: `${planDate}T08:00:00.000+08:00`
      });
      return { data: { ok: true }, entityType: 'source_scan_receipt', entityId: bindingId };
    });

    // E8：无新入库 → 资料门 no-op。
    const gateRun = buildDailyGateRun(database, started);
    assert.equal(gateRun.lane, LANE);
    assert.equal(gateRun.autoRelevant.length, 0);
    assert.equal(gateRun.pending.length, 0);
    const applied = await applyDailyLaneGate(runtime, started, gateRun, '无待判资料', agentRequestId(started.id, 'plan'), now);
    assert.equal(applied.archivedCount, 0);
    assert.equal(applied.relevantIds.size, 0);
    assert.equal(database.prepare('SELECT COUNT(*) count FROM source_lane_judgments').get().count, 0, '零新入库 → 判定零写');

    // 空方案成功路径（AC-017）：模型输出空 items → 保存 → 任务照常收尾 succeeded。
    const lease = runtime.acquireWorkerLease(started.id);
    const grantId = await ensureAutomaticTaskGrant(runtime, started.id);
    ...
    const saved = await savePlanFromSynthesisOutput(runtime, started, sessionFile, agentRequestId(started.id, 'plan'), lease.leaseId, grantId);
    assert.equal(saved.itemCount, 0);
    assert.equal(saved.filteredCount, 0);
    const completed = await dispatchCompleteAgentTask(runtime, started.id, { actor: owner, requestId: `complete-${randomUUID()}`, taskId: started.id });
    assert.equal(completed.status, 'succeeded', '零更新 + 空方案照常收尾为 succeeded（AC-017 空跑不变）');
