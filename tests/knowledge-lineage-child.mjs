...
import {
  addKnowledgeCanvasNode,createContentProjectFromBriefIdempotent,createCreativeBriefIdempotent,
  createKnowledgeCanvas,decideKnowledgeSuggestionIdempotent,getCreativeBriefLineage,getKnowledgeCanvas
...
try{
  ...
  const created=createCreativeBriefIdempotent(db,{requestId:'brief',canvasId:canvas.id,nodeIds:[topicNode.id,sourceNode.id],selectionMode:'selected',title:'直接简报',coreJudgment:'核心判断',whyNow:'现在值得讲',structure:['结论','证据','边界'],evidenceNodeIds:[sourceNode.id]});
  const confirmed=(await import('../src/main/knowledge-canvas.ts')).updateCreativeBriefIdempotent(db,{requestId:'confirm',id:created.data.id,expectedRevision:1,title:'直接简报',coreJudgment:'核心判断',whyNow:'现在值得讲',structure:['结论','证据','边界'],evidenceNodeIds:[sourceNode.id],status:'confirmed'});
  const linked=createContentProjectFromBriefIdempotent(db,{requestId:'project',briefId:created.data.id,expectedRevision:confirmed.data.revision});
  const replay=createContentProjectFromBriefIdempotent(db,{requestId:'project',briefId:created.data.id,expectedRevision:confirmed.data.revision});
  if(!replay.replayed||linked.data.project.id!==replay.data.project.id||linked.data.project.sourceIds[0]!==source.id||linked.data.project.topicId!==topic.id)throw new Error('brief project linkage failed');
  ...
  const decided=decideKnowledgeSuggestionIdempotent(db,{requestId:'confirm-return',id:pending[1].id,expectedRevision:pending[1].revision,decision:'confirm'});
