 ...
 * 锚定本批次（5135 前基线）能力/角色/schema 快照，逐项可证伪：
 * - exact 冻结面：三表 schema、五角色目录、红线命令、红线三能力属性、页透传、
 *   批内 intent 所需能力、基建 grant 命令——任一改动即失败；
 * - 子集面：核心能力（11 项）与五角色 standing 写权/读面按「批内基线 ⊆ 派生值」校验，
...
const CORE_CAPABILITIES = Object.freeze([
  ...
  Object.freeze({
    id: 'cap.desk', displayName: '桌助知答', description: '页级作用域透传；standing 写权为空',
    ...
    precise: false, agentGrantable: true, pageScopePassThrough: true, owner: 'desk', since: '2026-08-07'
  }),
  Object.freeze({
...
// —— 五角色 standing 写权（批内基线；5150 只增的 topic_maintenance_propose 不在此锚定）——
...
// —— 页透传（exact 冻结）——
const ROLE_PASSTHROUGH_SNAPSHOT = Object.freeze({ desk: true, librarian: false, planner: false, reporter: false, writer: false });
...
export function assertCompatibilityInvariants({ database }) {
  ...
  // —— 红线属性不漂移：红线命令冻结，红线条目保持 Precise + Owner-only（零角色绑定/零 grant 通道）——
  ...
  // —— 新增能力不扰动本批次交集面：五角色 standing 写权/读面保留批内基线、页透传、批内 intent 所需能力、基建 grant 命令 ——
  ...
  for (const [role, pass] of Object.entries(ROLE_PASSTHROUGH_SNAPSHOT)) {
    assert.equal(roleHasPagePassThrough(role), pass, `${role} 页透传不漂移`);
  }
