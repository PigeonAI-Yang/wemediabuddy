...
export function DomainMapView({selectedDomainId,onSelectDomain,onOpenTopic}:{selectedDomainId:string|null;onSelectDomain:(id:string|null)=>void;onOpenTopic:(id:string)=>void}){
  ...
  return <section className="ks-domains">
    <div className="ks-domain-toolbar"><input aria-label="搜索领域或主题" placeholder="搜索领域、主题或判断" value={query} onChange={event=>setQuery(event.target.value)}/><select aria-label="领域状态" value={status} onChange={event=>setStatus(event.target.value)}><option value="">全部状态</option><option value="active">活跃</option><option value="watching">持续观察</option><option value="dormant">长期沉寂</option></select><select aria-label="领域排序" value={order} onChange={event=>setOrder(event.target.value as any)}><option value="recent">按近期变化</option><option value="size">按资产规模</option><option value="manual">按手工顺序</option></select><div className="ks-mode" role="group" aria-label="领域显示模式"><button className={mode==='map'?'active':''} onClick={()=>setMode('map')}>地图</button><button className={mode==='list'?'active':''} onClick={()=>setMode('list')}>列表</button></div><button className="primary-button" onClick={()=>setEditor({title:'',description:'',status:'active',topicIds:[]})}>建立领域</button></div>
    ...
    <div className={`ks-domain-content ${mode}`}>
      ...
      {mode==='map'&&<div className="domain-grid">{page.items.map((domain:any)=><section className={`domain-card dc-${domain.status}`} key={domain.id} onClick={()=>{onSelectDomain(domain.id);setOpened(details[domain.id]??domain);}}>
        <span className={`pill-status ${domain.status==='active'?'green':domain.status==='watching'?'amber':'gray'}`}><span className="dot"/>{domain.status==='active'?'活跃产出':domain.status==='watching'?'持续观察':'休眠'}</span>
        <div className="domain-name">{domain.title}</div>
        <div className="domain-desc">{domain.description||'尚未填写领域说明'}</div>
        <div className="domain-stats"><span><b>{domain.topicCount}</b> 主题</span><span><b>{domain.sourceCount}</b> 资料</span><span className="domain-signal">{signal(domain)}</span></div>
        <div className="domain-actions"><button onClick={(event)=>{event.stopPropagation();edit(domain);}}>编辑</button><button onClick={(event)=>{event.stopPropagation();void archive(domain);}}>归档</button></div>
      </section>)}</div>}
      {mode!=='map'&&page.items.map((domain:any)=><section className="ks-domain-row" key={domain.id}><div className="ks-domain-title"><button onClick={()=>{onSelectDomain(domain.id);setOpened(details[domain.id]??domain);}}><h2>{domain.title}</h2></button><p>{domain.description||'尚未填写领域说明'}</p><small>{domain.topicCount} 个主题 · {domain.sourceCount} 条资料</small><div><button onClick={()=>edit(domain)}>编辑</button><button onClick={()=>void archive(domain)}>归档</button></div></div><div className="ks-clusters">
        {(details[domain.id]?.topics??[]).slice(0,100).map((topic:any)=><button className={`ks-cluster ${topic.status}`} key={topic.id} onClick={()=>{onSelectDomain(domain.id);onOpenTopic(topic.id);}}><header><i/><b>{topic.title}</b><span>{topic.lastSeenAt?new Date(topic.lastSeenAt).toLocaleDateString('zh-CN'):'尚无资料'}</span></header><p>{topic.summary||'尚未形成稳定判断'}</p><footer><span>{topic.sourceCount} 条资料</span><span>{topic.opportunityCount} 个机会</span></footer></button>)}
