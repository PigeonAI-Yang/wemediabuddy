...
import type { XListPost, XListPostDetail } from '../main/platforms/x-list-browser-types';
import type { XListTimelineCacheRecord } from '../main/x-list-timeline-cache';
...
declare global {
  interface Window {
    wmb: {
      ...
      readXListTimeline(input: { listId: string; limit?: number; knownUrls?: string[] }): Promise<{ accountKey: string; detail: { listId: string; canonicalUrl: string; name: string; ownerHandle: string | null; kind: 'owned' | 'following' | 'member' | 'unknown'; description: string; isPrivate: boolean; memberCount: number | null; observation: { capturedAt: string; pageUrl: string; fingerprint: string; visibleText: string } }; posts: XListPost[]; hasMore: boolean; livePostCount?: number; refreshDisposition?: 'retained_cache' | 'merged_cache' | 'updated' }>;
      readXListPost(input: { statusUrl: string; replyLimit?: number; bypassCache?: boolean }): Promise<{ accountKey: string; post: XListPostDetail; cached: boolean; fetchedAt: string; stale: boolean }>;
      getCachedXListTimeline(input: { accountKey: string; listId: string }): Promise<XListTimelineCacheRecord | null>;
