...
export function XListOperationModal(): React.JSX.Element | null {
  ...
  // Keep callbacks above the empty-state return: polling can add an operation after the first render.
  ...
  const target = `${operation.accountKey} · ${operation.snapshot.list?.name ?? operation.listId ?? '新 List'}`;

  return <><button ref={triggerRef} type="button" className="x-list-operation-trigger" aria-controls="x-list-operation-modal-dialog" aria-expanded={open} onClick={() => setOpen((value) => !value)}>X List · {status(operation)}{operation.items.length ? ` ${done}/${operation.items.length}` : ''}</button>
    <AppModal open={open} title={labels[operation.kind]} size="confirm" role="alertdialog" testId="x-list-operation-modal" className={`x-list-operation-modal state-${operation.state}`} ariaDescription={target} closeOnBackdrop={false} closeOnEscape={!busy} initialFocusRef={cancelRef} returnFocusRef={triggerRef} onRequestClose={requestClose}
      footer={<>
        <button ref={cancelRef} type="button" className="x-list-operation-cancel" disabled={busy} onClick={requestClose}>取消</button>
        {canConfirm && <button type="button" className="x-list-operation-confirm" disabled={busy} onClick={() => void confirm()}>确认执行</button>}
        {operation.state === 'running' && <button type="button" className="x-list-operation-stop" disabled={busy} onClick={() => void stop()}>停止后续成员</button>}
      </>}>
      <p>{target}</p>
