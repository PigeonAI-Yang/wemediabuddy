...
function WorkspaceStep({ busy, createDefault, choose }: { busy: boolean; createDefault: () => void; choose: () => void }): React.JSX.Element {
  return <div className="onboarding-card"><span className="eyebrow">STEP 01</span><h2>选择内容与数据住在哪里</h2><p className="onboarding-lead">推荐使用“文档”里的 WeMediaBuddy 文件夹。数据库、素材、导出和日志都在这里，随时可备份。</p><div className="onboarding-choice featured"><div><span className="choice-kicker">推荐</span><h3>创建默认工作空间</h3><p>文档\WeMediaBuddy</p></div><button className="onboarding-primary" disabled={busy} onClick={createDefault}>{busy ? '正在创建…' : '一键创建'}</button></div><div className="onboarding-choice"><div><h3>使用自定义目录</h3><p>选择新目录，或接入已有 WeMediaBuddy 数据根。</p></div><button className="secondary-button" disabled={busy} onClick={choose}>选择目录</button></div></div>;
}
...
function PlatformsStep({ status, busy, onBusy, onNote, refresh, finish }: { status: OnboardingStatus; busy: boolean; onBusy: (value: boolean) => void; onNote: (value: string) => void; refresh: () => Promise<unknown>; finish: () => Promise<void> }): React.JSX.Element {
  ...
  const cards = [
    ...
    { id: 'x', name: 'X', detail: '在专用浏览器中检测当前账号', action: () => verify('x') },
    { id: 'wechat', name: '微信公众号', detail: '检测公众号后台登录状态', action: () => verify('wechat') }
  ];
