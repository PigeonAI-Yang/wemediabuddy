  ...
  try {
    ...
    await runPiPromptWithFallback({
      ...
      run: async (activeRuntime) => {
        const result = await activeRuntime.promptUntilSettled(libraryOrganizePrompt(task, ctx), { timeoutMs: promptTimeoutMs() });
