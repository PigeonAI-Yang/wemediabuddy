export type ErrorCode = 'REVISION_CONFLICT' | 'INVALID_STATE' | 'ACCOUNT_MISMATCH' | 'CONFIRMATION_REQUIRED' | 'CONFIRMATION_STALE' | 'EXECUTION_GRANT_REQUIRED' | 'EXECUTION_GRANT_SCOPE_MISMATCH' | 'BROWSER_NEEDS_USER' | 'PUBLICATION_UNKNOWN' | 'X_LIST_UNKNOWN' | 'METRIC_UNAVAILABLE' | 'NOT_FOUND' | 'VALIDATION_ERROR' | 'WORKSPACE_BUSY' | 'WORKSPACE_NOT_FOUND' | 'WORKSPACE_ID_MISMATCH' | 'WORKSPACE_SWITCH_FAILED' | 'PROFILE_STALE' | 'OFFICIAL_PACK_UNAVAILABLE' | 'PI_RUNTIME_PROBE_FAILED' | 'HAS_PLATFORM_VERSIONS' | 'HAS_CONTEXT_USES' | 'HAS_CANVAS_REFS' | 'PLAN_ITEM_NOT_APPROVED' | 'THESIS_LOCK_REQUIRED' | 'THESIS_LOCK_VIOLATION';
