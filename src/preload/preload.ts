...
contextBridge.exposeInMainWorld('wmb', {
  ...
  savePiConfig: (input: { id?: string; name: string; baseUrl: string; model: string; api: 'openai-responses' | 'anthropic-messages'; apiKey?: string }) => ipcRenderer.invoke('pi-config:save', input),
  ...
  listPiModels: (input: { id?: string; baseUrl: string; api: 'openai-responses' | 'anthropic-messages'; apiKey?: string }) => ipcRenderer.invoke('pi-config:list-models', input) as Promise<string[]>,
