...
- 纯模型发布、参数/价格新闻只有翻译成新受众可执行的决策/行动并挂接个人商业化时才进内容，否则降权或排除；
...
- 标题可以有一点钩子，但必须是直白人话：写 `title` 前在内部生成至少三个不同切口（具体问题 / 关键动作或方法 / 对象或证据冲突），再选最能被来源兑现的一条；`title` 必须包含题材独有的问题、动作、对象或证据，不靠上下文就能读懂、可直接发布。不得使用「普通人」等万能受众标签，不复用近期标题的固定前缀与句式；来源未支持的数字、结果和因果不得进入标题。夸张、反常识、需要解释的爆点只放 `titleGuidance`。禁止血腥/身体/歧义比喻（如“说不出话”）和需要二次解读的措辞；开头必须有爆点，观点要把远方的愿景拉近；
