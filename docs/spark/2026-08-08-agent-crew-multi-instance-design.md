...
- 日期：2026-08-08（Owner 显式 UI 选择确认）；2026-08-09 落档并依 Owner 追加五项选择与独立复审修正
- 作者：Designer（依据 Owner 2026-08-08 会话确认方向统稿）
- 状态：**Owner 设计确认（蓝图）**。本文件只落档班组协同方向；不授予任何代码施工权。施工须另行任务合同与施工 Owner lock。
- Route：Design。本阶段不修改 PRODUCT.md / PRD.md / SPEC.md / PLAN.md / TASKS.md / 源码 / 测试 / 台账。
...
5. **历史保留策略**：默认保留 agent_tasks 既有语义（interrupted 续跑）+ 会话文件随项目数据；不做新表、不做归档流水线（蓝图期不新增持久化）。
...
- 零改动：不修改产品文档、源码、测试、台账、合同（§17 无施工权）。
...
**本块记录 Owner 2026-08-08 显式 UI 选择确认 + 2026-08-09 追加五项选择与独立复审确认的班组协同方向。它是设计确认（蓝图），不是施工 Owner lock：本文件不授予任何代码施工权；任何施工须另行任务合同 + 施工 Owner lock 授权。**

| # | 确认点 | 落点 |
| --- | --- | --- |
| 1 | **固定五角色**：桌助/记者/策划（兼复盘）/写手/资料员，跨赛道稳定 | §0.1、§3.1、PRODUCT C8 |
| 2 | **同角色多实例显式可见**：同角色可同时多实例，活动视图逐实例呈现 | §4、§5.5、§14 A1 |
| 3 | **实例按任务创建，任务结束后退出活动视图**（needs_user 停留例外见 #9） | §5、§14 A2/A3 |
| 4 | **不预设空槽**：活动视图无实例即无渲染（空态呈现细化见 #10） | §4、§11.1、§14 A4 |
| 5 | **共享并发池仅作为系统容量**：maxWorkers（0..7，0=停用派工）是容量，不是角色配额 | §9.1、§14 A5/A6 |
| 6 | **桌助是协调入口，不是主管工位**：主管 = 人；桌助无 standing 写权、不进员工槽、不代批 | §10、§14 A7 |
| 7 | **实例权限 = 任务精确授权 ∩ 角色能力 ∩ 资源边界**（对象级硬隔离见 #12） | §8、§14 A8/A9 |
| 8 | **发布/硬删红线不变**：最终发布点击与硬删仅人类 UI；平台副作用仅 Precise + Owner UI | §8.4、§14 A10 |
| 9 | **needs_user 停留活动区**：终态 needs_user 卡留在活动视图「等你批」，直至用户处理/关闭；期间不占并发、不持 lease/grant/锁 | §3.1、§5、§6.5、§11.1、§14 A3/A11 |
| 10 | **空态呈现（细化 #4）**：五角色分组始终可见；空角色显示「当前无任务」，不画空槽 | §0.3、§3.2、§4、§11.1、§14 A2/A4 |
| 11 | **角色编号仅活动期显示**：实例显示「记者1/记者2」角色编号，退出活动视图即不再以编号指认；持久身份仍 jobId | §7.2、§14 A1 |
| 12 | **对象级硬隔离（施工必需）**：dispatcher 必须校验工单对象边界，跨对象写请求拒绝 | §8.1、§12.2.7、§14 A9 |
| 13 | **持久续派合同**：jobId/roleId/brief/边界参数写入既有 context_refs_json，重启后完整指认与一键续派，不新增表 | §7.3、§12.2.1、§14 A13 |
