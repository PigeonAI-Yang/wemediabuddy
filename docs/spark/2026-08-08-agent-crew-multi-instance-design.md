...
- **2026-08-10 主管授权翻转修订（legislative）**：依 `docs/spark/2026-08-10-supervisor-authority-design.md` §8（Owner lock 2026-08-10 全 10 项确认），本文件下列条款翻转——§0.1-5、§1.1 P4（转归档）、§2.1-5、§3.2-5、§8.3、§10、§14 A7、§17-6。**主管（desk）= 软件内主管/主编席，持全站内部 standing 写权（全部 grantable 业务能力命令 ∪ INFRA，含内部准备命令，不含三类红线执行命令）**；红线恰三类（最终平台发布、硬删执行、外部平台变更执行），最终动作仅人类 UI 新鲜确认；内部审批归主管；`spawn(roleId:'desk')` 仍拒绝。历史 Owner lock 文本（§17）保留，本修订以显式注记叠加；旧「桌助 = 协调入口 / 无 standing 写权」表述仅作历史引证。
- 作者：Designer（依据 Owner 2026-08-08 会话确认方向统稿）
- 状态：**正式施工 Owner lock（2026-08-08 Owner 会话确认并落档升级；2026-08-10 主管授权翻转已并入）**。本文件是班组多实例协同方向的唯一设计真源。**TASKS doing 仍是唯一施工许可**：本文件不替代任务合同、不直接授予施工权；任何施工须另行任务合同（显式引用 §12 兼容原则与 §16 影响面）并经 TASKS doing 授权（§17）。
...
5. **主管（desk）= 软件内主管 / 主编席（2026-08-10 主管授权翻转）**：真正的「人」主管职责 = 三类红线最终动作的新鲜确认与最终担责（定目标、担责）。主管（desk）是对人说话的入口与单跳派工执行者：派工、盯梢、传话、呈报、**内部审批**；主管持全站内部 standing 写权（全部 grantable 业务能力命令 ∪ INFRA，含内部准备命令，不含三类红线执行命令）、永不进员工槽、红线最终动作不代批（须人类 UI 新鲜确认）。
...
| P4 | **主管工位漂移（归档，2026-08-10 翻转）** | desk = 软件内主管（standing 全量内部写权） | 旧「防 desk 变主管」已按 2026-08-10 主管授权翻转归档；现行防的是红线漂移（红线三类执行命令不可达任何 standing/page scope）与员工漂移（§15） |
...
5. 主管（desk）= **软件内主管/主编席**（对话、派工、盯梢、传话、呈报、内部审批），standing 全量内部写权（全部 grantable 业务能力命令 ∪ INFRA，含内部准备命令，不含三类红线执行命令）、不进员工槽、红线最终动作不代批（2026-08-10 主管授权翻转）。
...
5. **主管（desk）= 软件内主管/主编席（2026-08-10 翻转）**：standing 全量内部写权、永不进员工槽、单跳派工、传话/呈报 ≠ 红线代批（红线三类最终动作仅人类 UI 新鲜确认）。
...
智能体（班组）页 —— 一级导航（实例驱动的活动视图）
 ...
 │    策划 | 写手 | 资料员（同构）
 │    └─ 主管（desk）：唯一常驻主管卡（对话入口，非实例——见 §10；2026-08-10 身份翻转）
 ├─ 历史（可折叠，从持久面投影）
...
3. **主管（desk）**：standing 全量内部写权（`commandsCoveredByGrantableCapabilities() ∪ INFRA_GRANT_COMMANDS`，含内部准备命令与内部审批；不含三类红线执行命令；2026-08-10 翻转）；主管工具（roster/jobs/message + 全量内部写命令）走 MCP 编排面 + 业务命令表。
...
| 维度 | 主管（desk） | 员工实例（reporter/planner/writer/librarian） |
| --- | --- | --- |
| 本质 | 软件内主管（主编席；唯一常驻对话面） | 按任务的执行单元（subagent） |
| 创建 | 随应用存在（dock 默认收件人） | 按 spawn 创建，终态退出活动视图 |
| 会话 | 主 Pi 会话（交互式） | `job-<jobId>.jsonl` 独立会话，dock 不可直呼 |
| 写权 | standing 全量内部（全部 grantable 命令 ∪ INFRA，含内部准备/内部审批；不含三类红线执行命令；2026-08-10 翻转） | 任务精确授权 ∩ 角色能力 ∩ 资源边界 |
| 槽位 | 永不进 JobPool 员工槽 | 占共享池槽位 |
| 职责 | 接单、拆解、单跳派工、盯梢、传话、呈报、**内部审批** | 执行有界工单、业务读回、终态报告 |
| 决策 | 内部审批归主管；红线三类最终动作不代批（人类 UI 新鲜确认） | 不决策选题、不自动转派 |

硬边界：

1. **主管（desk）是软件内主管/主编席（2026-08-10 翻转）**：持全站内部 standing 写权（全部 grantable 业务能力命令 ∪ INFRA，含内部准备命令，不含三类红线执行命令）；永不进员工槽、`spawn(roleId:'desk')` 被拒（`ROLE_NOT_SPAWNABLE`）；内部审批归主管；红线三类最终动作不代批（须人类 UI 新鲜确认）。
2. **单跳有界**：主管一次只给一个员工一张有界单；接力（扫完 → 派策划）由主管主循环执行，员工实例之间不互聊、不转派。
3. **会话不混流**：员工会话内容不进入 dock 转录；每实例独立上下文；实例间无隐式记忆（§5.6）。
4. **同一事实源**：主管对进度/状态的回答只来自投影 API（roster/jobs/task），禁止编造。
...
| A7 | 主管非员工槽（2026-08-10 翻转） | desk standing = 全量内部命令（含内部审批；不含三类红线执行命令）；`spawn(roleId:'desk')` 被拒；红线最终动作仅人类 UI |
...
| 6 | **主管（desk）= 软件内主管/主编席（2026-08-10 主管授权翻转，历史锁 2026-08-08 第 6 点「桌助是协调入口，不是主管工位：主管 = 人；桌助无 standing 写权、不进员工槽、不代批」保留为历史并由此修订）**：主管 standing 写权 = 全量内部（全部 grantable 业务能力命令 ∪ INFRA，含内部准备命令，不含三类红线执行命令）、不进员工槽、内部审批归主管、红线最终动作不代批 | §0.5、§10、§14 A7、PRODUCT C9.6、PRD §2.4.6、SPEC CAP-027.6 |
