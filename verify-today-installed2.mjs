...
async function run() {
  ...
  // click Today if not already on Today
  ...
  if (!clicked) console.log('no Today nav click, current url maybe already Today');
  ...
  try { await page.waitForSelector('.app-shell', { timeout: 5000 }); console.log('app-shell found'); } catch { console.log('app-shell NOT found'); }

  const piSelectors = ['.pi-dock', '[data-testid="pi-dock"]', 'text=Pi', '.pi-open'];
  for (const sel of piSelectors) {
    try { const c = await page.locator(sel).count(); console.log(`pi selector ${sel} count ${c}`); } catch {}
  }
  ...
  const domChecks = await page.evaluate(() => {
    ...
    return { hasHeader, hasEmpty, appShell, todayLayout, todayOverview, piDock, piOpen, bodySnippet };
  });
  console.log('domChecks', JSON.stringify(domChecks, null, 2));
