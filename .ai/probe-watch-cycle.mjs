...
const cdpPort = 9360;
const shot = path.join(root, '.ai', 'today-cycle-complete.png');
...
for (let i = 0; i < 96; i++) {
  ...
  state = await page.evaluate(async (businessDate) => {
    ...
    return {
      ...
      planSummary: (data?.plan?.summary || '').slice(0, 80),
  ...
  const active = /正在启动|正在连接|正在生成|正在整理|正在核验|正在扫描/.test(state.runningText);
