...
## Goal
transient controls foundation：pool `RESOURCE_JUDGE_IN_FLIGHT` 三码泊车 + skip-self、`JobExecutionOutcome` 瞬时 deferred 类型、`job-control.ts` 抽取 cancel 序列与 stoppable 注册协议、job-spawner 拆分与 line-caps 登记。

## Acceptance
- [ ] `RESOURCE_WAIT_CODES` 含第三码 `RESOURCE_JUDGE_IN_FLIGHT` 且 `park` 接受三码（锁冲突 / lease 忙 / judge in flight），FIFO 序保持
- [ ] judge-park 工单自身泊车级联不原地拉起（skip-self，同 lease-busy 模式），不空转
- [ ] `JobExecutionOutcome.status` 含瞬时 `'deferred'` 且不在 `JobTerminalStatus`；deferred 传 `mapOutcomeToTerminal` 为类型错误（编译期保证不误映射）
- [ ] `runCancellationSequence` 与原 `cancel()` 行为等价：queued / waiting / running 三态取消、重复取消幂等、MINOR 3 事件去重
- [ ] job-spawner 拆分后行数 < 523 且 `scripts/line-caps.json` 同步只降登记
- [ ] tests/job-pool + tests/job-spawner 全绿

## Allowed paths
- src/main/job-pool.ts（`RESOURCE_WAIT_CODES` / `park` / `nextCandidate`）
- src/main/role-job-registry.ts（`JobExecutionOutcome` 瞬时 deferred 类型 + `JOB_ERROR_CODES` 码）
- src/main/job-control.ts（新增：`runCancellationSequence` + `createStoppableRegistrar`）
- src/main/job-spawner.ts（cancel 抽取 + registerStoppable 接线，薄委托）
- scripts/line-caps.json（只降登记）
...
no change — agent-capabilities.ts / page-authority.ts 零改动；由 check:capabilities（A1）+ librarian effective grant 一致性（A2）验证。Pi operator Skill impact: no change — 池内部机制（deferred 车道/取消注册协议）非提示词语义，按 docs/pi-operation-skill-maintenance.md 注明。
